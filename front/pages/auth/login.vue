...
const handleLogin = async () => {
  const { error } = await useAuth().login(form.value);
  console.log(error);
};
...
const handleLogout = async () => {
  try {
    ...
    // thenの中で以下コードを書いたら２回ログアウトしないとcookie削除されなかった
...
//   if (process.server) {
//     // headers = { ...useRequestHeaders(['cookie']), referer: config.public.referer };
//     // ssr時にはコンテナ名でアクセスする必要がある
//     // 後にssr,csrでエンドポイントを切り替えるプラグイン実装
//     // let testEndPoint = 'http://nginx:80' + '/api/test';
//     let testEndPoint = '/api/test';
//     console.log('ssr-fetch');
//     // headers['Accept'] = 'application/json';
//     // console.log(testEndPoint);
//     const { data: res } = await useAsyncData('ssr-test', () => {
//       return useApi().fetcher()('/api/test', {
//         method: 'GET',
//       });
//     });
...
//     return res;
//   }
//   const { data: res } = await useAsyncData('test', () => {
//     return useApi().fetcher()('/api/test', {
//       method: 'GET',
//       // credentials: 'include',
//     });
            ...
            <div class="mb-6 flex items-center justify-between">
              <div class="mb-[0.125rem] block min-h-[1.5rem] pl-[1.5rem]">
                <input
                  ...
                  checked
                />
                <label class="inline-block pl-[0.15rem] hover:cursor-pointer" for="exampleCheck3"> Remember me </label>
              </div>
